...
export const apiClient =
  {
    ...
    > {
      ...
      try {
        console.log(
          "🔍 APIClient: Getting auth session...",
        );
        const session =
          await auth();
        console.log(
          "👤 APIClient: Session exists:",
          !!session?.user,
        );
        console.log(
          "🔑 APIClient: JWT token exists:",
          !!session
            ?.user
            ?.jwtToken,
        );
        ...
        if (
          ...
          console.log(
            "⚠️ APIClient: No JWT token available, proceeding without auth",
          );
        }
      } catch (error) {
        console.error(
          "❌ APIClient: Failed to get auth session:",
          error,
        );
      }
